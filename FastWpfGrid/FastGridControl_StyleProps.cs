...
public partial class FastGridControl {
	...
	private Color _headerBackground = Color.FromRgb(230, 230, 230);
	...
	private Color _limitedSelectedColor = Color.FromRgb(51, 153, 255);/*Color.FromRgb(51, 220, 220);*/
	private Color _limitedSelectedTextColor = Colors.White;
	private Color _mouseOverRowColor = Color.FromRgb(240, 240, 240); // Colors.LemonChiffon; // Colors .Beige;
	private string _cellFontName = "Arial";
	//private readonly int _cellFontSize;
	private Color _gridLineColor = Colors.LightGray;
	private int _cellPaddingHorizontal = 2;
	//private readonly int _minColumnWidth = 0;
	...
	//private int _wideColumnsLimit = 2500000;

	public int ColumnResizeThreshold {
		get => _columnResizeThreshold;
	...
	public static readonly DependencyProperty CellFontSizeProperty = DependencyProperty.Register(
		...
		new PropertyMetadata(11, OnCellFontSizeChanged)
	);

	private static void OnCellFontSizeChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
		FastGridControl view = (FastGridControl)d;

		//view.RecountColumnWidths();
		view.RecountRowHeights();
		view.AdjustScrollbars();
		view.SetScrollbarMargin();
		view.FixScrollPosition();
		view.RecalculateDefaultCellSize();
		view.InvalidateAll();
	}
	...
	public int AutoSizeExtraPadding {
		get => (int)GetValue(AutoSizeExtraPaddingProperty);
		set => SetValue(AutoSizeExtraPaddingProperty, value);
	}

	public static readonly DependencyProperty AutoSizeExtraPaddingProperty = DependencyProperty.Register(
		nameof(AutoSizeExtraPadding),
		typeof(int),
		typeof(FastGridControl),
