...
public partial class FastGridControl {
	...
	private int RenderBlock(int leftPos, int rightPos, Color? selectedTextColor, Color bgColor, IntRect rectContent, IFastGridCellBlock block, FastGridCellAddress cellAddr, bool leftAlign, bool isHoverCell) {
		...
		if (renderBlock && block.CommandParameter != null) {
			IntRect activeRect = new IntRect(new IntPoint(leftAlign ? leftPos : rightPos - width, top), new IntSize(width, height)).GrowSymmetrical(1, 1);
		...
		switch (block.BlockType) {
			case FastGridBlockType.Text:
				if (renderBlock) {
					IntPoint textOrigin = new(leftAlign ? leftPos : rightPos - width, top);
					GlyphFont font = GetFont(block.IsBold, block.IsItalic);
					_drawBuffer.DrawString(textOrigin.X, textOrigin.Y, rectContent, selectedTextColor ?? block.FontColor ?? CellFontColor, UseClearType ? bgColor : null,
										   font,
										   block.TextData);
				}
				break;
			case FastGridBlockType.Image:
				if (renderBlock) {
					IntPoint imgOrigin = new(leftAlign ? leftPos : rightPos - block.ImageWidth, top);
					ImageHolder image = GetImage(block.ImageSource);
