...
/// <summary>
/// Manager to hold column/row sizes and indexes <br />
/// Terminology:  <br />
/// ModelIndex - index in model <br />
/// ScrollIndex - index in scroll are (=RealIndex-FrozenCount) <br />
/// FrozenIndex - index in frozen area <br />
/// RealIndex - index in frozen and scroll area (first are frozen items, than scroll items) <br />
/// Grid uses mostly RealIndex
/// </summary>
public class SeriesSizes : IDisposable {
	...
	public int GetAllFrozenHeight() {
		if (_frozenItems.Count == 0) {
			return 0;
		}
		return _frozenItems.OrderByDescending(x => x.EndPosition).First().EndPosition;
	}
	...
	public void PutSizeOverride(int modelIndex, int size) {
		...
		if (!_sizeOverridesByModelIndex.ContainsKey(modelIndex)) {
			_sizeOverridesByModelIndex[modelIndex] = size;
		}

		if (size > _sizeOverridesByModelIndex[modelIndex]) {
			_sizeOverridesByModelIndex[modelIndex] = size;
		}
	}
	...
	public int GetScrollIndexOnPosition(int position) {
		...
		if (position <= _scrollItems[itemOrder - 1].EndPosition) {
			return _scrollItems[itemOrder - 1].ScrollIndex;
		}
	...
	public int GetFrozenIndexOnPosition(int position) {
		foreach (SeriesSizeItem item in _frozenItems) {
			if (position >= item.Position && position <= item.EndPosition) {
				return item.FrozenIndex;
			}
		}
		return -1;
	}
	...
	public int GetSizeSum(int startScrollIndex, int endScrollIndex) {
		...
		for (int i = order1; i <= order2; i++) {
			...
			SeriesSizeItem item = _scrollItems[i];
	...
	public int GetSizeByModelIndex(int modelIndex) {
		if (_sizeOverridesByModelIndex.ContainsKey(modelIndex)) {
			return _sizeOverridesByModelIndex[modelIndex];
		}
	...
	public int GetTotalScrollSizeSum() {
		List<KeyValuePair<int, int>> scrollSizeOverrides = _sizeOverridesByModelIndex.Where(x => ModelIndexIsInScrollArea(x.Key)).ToList();
	...
	public int GetPositionByRealIndex(int realIndex) {
		...
		if (realIndex < FrozenCount) {
			return _frozenItems[realIndex].Position;
		}
	...
	public int GetPositionByScrollIndex(int scrollIndex) {
		...
		if (order >= 0) {
			return _scrollItems[order].Position;
		}

		order = ~order;
		order--;
		if (order < 0) {
			return scrollIndex * DefaultSize;
		}
	...
	public int ScrollInView(int firstVisibleIndex, int scrollIndex, int viewportSize) {
		...
		while (res < viewportSize && testedIndex >= 0) {
			...
			if (res + size > viewportSize) {
				return testedIndex + 1;
			}
		...
		if (res >= viewportSize && testedIndex < scrollIndex) {
			return testedIndex + 1;
		}
	...
	public void ResizeNoBuild(int realIndex, int newSize) {
		if (realIndex < 0) {
			return;
		}
	...
	public void ResizeNoBuild(int newSize) {
		int[] keys = _sizeOverridesByModelIndex.Keys.ToArray();
		foreach (int item in keys) {
			_sizeOverridesByModelIndex[item] = newSize;
		}
	}

	public void SetExtraordinaryIndexes(HashSet<int> hidden, HashSet<int> frozen) {
		_hiddenAndFrozenModelIndexes = [.. hidden];
		_frozenModelIndexes = [.. frozen.Where(x => !hidden.Contains(x))];
		_hiddenAndFrozenModelIndexes.AddRange(_frozenModelIndexes);

		_frozenModelIndexes.Sort();
		_hiddenAndFrozenModelIndexes.Sort();

		if (!_hiddenAndFrozenModelIndexes.Any()) {
			_hiddenAndFrozenModelIndexes = null;
		}

		if (!_frozenModelIndexes.Any()) {
			_frozenModelIndexes = null;
		}
	...
	public int RealToModel(int realIndex) {
		if (_hiddenAndFrozenModelIndexes == null && _frozenModelIndexes == null) {
			return realIndex;
		}
		...
		if (realIndex < FrozenCount && _frozenModelIndexes != null) {
			return _frozenModelIndexes[realIndex];
		}

		if (_hiddenAndFrozenModelIndexes == null) {
			return realIndex;
		}

		realIndex -= FrozenCount;
		foreach (int hidItem in _hiddenAndFrozenModelIndexes) {
			if (realIndex < hidItem) {
				return realIndex;
			}
	...
	public int ModelToReal(int modelIndex) {
		...
		if (hiddenIndex >= 0) {
			return modelIndex - hiddenIndex + FrozenCount;
		}
	...
	public bool IsVisible(int testedRealIndex, int firstVisibleScrollIndex, int viewportSize) {
		if (testedRealIndex < 0) {
			return false;
		}

		if (testedRealIndex >= 0 && testedRealIndex < FrozenCount) {
			return true;
		}
	...
	public bool IsIndexFrozen(int index) {
		if (_frozenModelIndexes == null) {
			return false;
		}
		return _frozenModelIndexes.Contains(index);
	}

	public void Dispose() {
		_sizeOverridesByModelIndex?.Clear();
		_hiddenAndFrozenModelIndexes?.Clear();
		_frozenModelIndexes?.Clear();
		_scrollItems?.Clear();
		_positions?.Clear();
		_scrollIndexes?.Clear();
		_frozenItems?.Clear();
