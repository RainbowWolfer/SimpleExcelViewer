...
internal static unsafe partial class WriteableBitmapExtensions
{
	#region Normal line
	...
	{
		using BitmapContext context = bmp.GetBitmapContext();
		using BitmapContext penContext = penBmp.GetBitmapContext();
	...
	public static void DrawLinePenned(BitmapContext context, int w, int h, int x1, int y1, int x2, int y2, BitmapContext pen)
	{
		...
		int pw = size;
		Rect srcRect = new(0, 0, size, size);
		...
		Rect destRect = new(x, y, size, size);
	...
	{
		Rect viewPortWithOffset = new(viewPort.X - offset, viewPort.Y - offset, viewPort.Width + (2 * offset), viewPort.Height + (2 * offset));
